...
Таким образом, баланс операций сохраняется, но учитывая операции с отброшенными нулями, количество комбинаций для сдвига  вправо -больше (реальное значение числа уменьшается (/2) большее количество раз чем 1,5х+1), можно предположить, что последовательность будет сжиматься.
